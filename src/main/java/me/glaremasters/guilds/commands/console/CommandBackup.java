    ...
    actionHandler.addAction(issuer.getIssuer(), new ConfirmAction() {
      ...
      public void accept() {
        guilds.getCommandManager().getCommandIssuer(issuer.getIssuer())
            .sendInfo(Messages.BACKUP__STARTED);
        Guilds.newChain().async(() -> {
          try {
            BackupUtils.zipDir("guilds-backup-" + System.currentTimeMillis() + ".zip",
                guilds.getDataFolder().getPath());
          } catch (Exception e) {
            e.printStackTrace();
          }
        }).sync(() -> guilds.getCommandManager().getCommandIssuer(issuer.getIssuer())
            .sendInfo(Messages.BACKUP__FINISHED)).execute();
