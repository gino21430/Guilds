...
public class CommandMigrate extends BaseCommand {
  ...
  public void execute(CommandIssuer issuer, @Values("@sources") String toBackend)
      throws IOException {
    if (issuer.isPlayer()) {
      return;
    }
      ...
      public void accept() {
        ...
        Guilds.newChain().async(() -> {
          try {
            ...
            resolvedAdapter.getCooldownAdapter()
                .saveCooldowns(cooldownHandler.getCooldowns().values());
          ...
          guilds.getCommandManager().getCommandIssuer(issuer.getIssuer())
              .sendInfo(Messages.MIGRATE__COMPLETE, "{amount}",
                  String.valueOf(guildHandler.getGuildsSize()));
