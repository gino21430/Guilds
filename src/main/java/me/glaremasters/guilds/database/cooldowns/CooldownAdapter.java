...
public class CooldownAdapter {
  ...
  public boolean cooldownExists(Cooldown.Type cooldownType, UUID cooldownOwner) throws IOException {
    return provider
        .cooldownExists(sqlTablePrefix, cooldownType.getTypeName(), cooldownOwner.toString());
  }

  public void createCooldown(Cooldown cooldown) throws IOException {
    createCooldown(cooldown.getCooldownType(), cooldown.getCooldownOwner(),
        cooldown.getCooldownExpiry());
  }

  public void createCooldown(Cooldown.Type cooldownType, UUID cooldownOwner, Long cooldownExpiry)
      throws IOException {
    provider.createCooldown(sqlTablePrefix, cooldownType.getTypeName(), cooldownOwner.toString(),
        new Timestamp(cooldownExpiry));
  }
