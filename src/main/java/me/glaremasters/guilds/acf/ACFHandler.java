...
public class ACFHandler {
  ...
  public ACFHandler(Guilds guilds, PaperCommandManager commandManager) {
    ...
    commandManager.getCommandReplacements().addReplacement("guilds",
        guilds.getSettingsHandler().getSettingsManager()
            .getProperty(PluginSettings.PLUGIN_ALIASES));
        ...
        if (file.isFile()) {
          if (file.getName().endsWith(".yml")) {
            ...
            commandManager.getLocales()
                .loadYamlLanguageFile(new File(languageFolder, file.getName()),
                    Locale.forLanguageTag(updatedName));
          }
        }
      }
      commandManager.getLocales().setDefaultLocale(Locale.forLanguageTag(
          guilds.getSettingsHandler().getSettingsManager()
              .getProperty(PluginSettings.MESSAGES_LANGUAGE)));
   ...
   */
  public void registerCustomContexts(PaperCommandManager commandManager,
    ...
    commandManager.getCommandContexts().registerIssuerOnlyContext(GuildRole.class, c -> {
      ...
      return guildHandler
          .getGuildRole(guild.getMember(c.getPlayer().getUniqueId()).getRole().getLevel());
    });
  }
   ...
   */
  public void registerCustomCompletions(PaperCommandManager commandManager,
      GuildHandler guildHandler, ArenaHandler arenaHandler) {
    commandManager.getCommandCompletions().registerCompletion("online",
        c -> Bukkit.getOnlinePlayers().stream().map(HumanEntity::getName)
            .collect(Collectors.toList()));
    commandManager.getCommandCompletions()
        .registerCompletion("invitedTo", c -> guildHandler.getInvitedGuilds(c.getPlayer()));
    commandManager.getCommandCompletions()
        .registerCompletion("joinableGuilds", c -> guildHandler.getJoinableGuild(c.getPlayer()));
    commandManager.getCommandCompletions()
        .registerCompletion("guilds", c -> guildHandler.getGuildNames());
    commandManager.getCommandCompletions().registerCompletion("arenas",
        c -> arenaHandler.getArenas().stream().map(Arena::getName).collect(Collectors.toList()));
    commandManager.getCommandCompletions()
        .registerCompletion("locations", c -> Arrays.asList("challenger", "defender"));
    commandManager.getCommandCompletions().registerCompletion("languages",
        c -> guilds.getLoadedLanguages().stream().sorted().collect(Collectors.toList()));
    commandManager.getCommandCompletions()
        .registerCompletion("sources", c -> Arrays.asList("JSON", "MYSQL", "SQLITE"));
    ...
    commandManager.getCommandCompletions().registerCompletion("members", c -> {
      ...
      return guild.getMembers().stream().map(m -> Bukkit.getOfflinePlayer(m.getUuid()).getName())
          .collect(Collectors.toList());
    });
    ...
    commandManager.getCommandCompletions().registerAsyncCompletion("allyInvites", c -> {
      ...
      return guild.getPendingAllies().stream().map(guildHandler::getNameById)
          .collect(Collectors.toList());
    });
    ...
    commandManager.getCommandCompletions().registerAsyncCompletion("vaultAmount", c -> {
      ...
      return IntStream.rangeClosed(1, list.size()).mapToObj(Objects::toString)
          .collect(Collectors.toList());
    });
  ...
  public void registerDependencyInjections(PaperCommandManager commandManager) {
    ...
    commandManager.registerDependency(SettingsManager.class,
        guilds.getSettingsHandler().getSettingsManager());
